...
// ════════════════════════════════════════════════════════════════
//  LANDING PAGE — menu, modais, auth overlay
//  Essas funções são chamadas diretamente pelo HTML (onclick=),
//  por isso precisam estar aqui em app.js, que é o único arquivo
//  garantidamente carregado em toda página.
// ════════════════════════════════════════════════════════════════
...
// testemunhasAtivas declarada UMA vez aqui
...
// ════════════════════════════════════════════════════════════════
//  FIREBASE AUTH — listener único aqui
// ════════════════════════════════════════════════════════════════
...
function doLogin() {
  ...
  waitForFirebase(({ auth, signInWithEmailAndPassword }) => {
    signInWithEmailAndPassword(auth, email, pass)
      .catch(err => { btn.textContent = 'Entrar na conta'; btn.disabled = false; showAuthErr(errEl, fbMsg(err.code)); });
  });
}
...
function doRegister() {
  ...
  waitForFirebase(({ auth, createUserWithEmailAndPassword, updateProfile }) => {
    createUserWithEmailAndPassword(auth, email, pass)
      ...
      .catch(err => { btn.textContent = 'Criar conta grátis'; btn.disabled = false; showAuthErr(errEl, fbMsg(err.code)); });
  });
}
...
// ════════════════════════════════════════════════════════════════
//  FIRESTORE — salvar / buscar / atualizar / deletar
// ════════════════════════════════════════════════════════════════
...
async function saveDocFS(docObj) {
  return new Promise(resolve => {
    waitForFirebase(async ({ db, collection, addDoc }) => {
      try {
        const ref = await addDoc(collection(db, 'documents'), { ...docObj, userId: currentUser.uid });
        docObj.fsId = ref.id;
      } catch (e) { saveDocLocal(docObj); }
      resolve(docObj);
    });
  });
}

// FIX Bug 7: salva sem o HTML gigante no Firestore
  ...
  return new Promise(resolve => {
    waitForFirebase(async ({ db, collection, getDocs, query, where }) => {
      try {
        ...
        currentDocs = docs;
      } catch (e) { currentDocs = getDocsLocal(); }
      resolve(currentDocs);
    });
  });
}
...
const PAGE_TITLES = {
  ...
  settings:'Configurações', document:'Visualizar Documento', edit:'Editar Documento'
};
