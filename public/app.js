...
function gotoPage(page) {
  ...
  if (page === 'mydocs')    { loadDocsFS().then(() => renderDocs()); }
...
// ── Função central de chamada à IA (backend primeiro, fallback direto) ──
async function callIA({ system = '', messages = [], max_tokens = 1500, model = 'claude-sonnet-4-20250514' }) {
  // Tenta backend (Vercel/Netlify)
  try {
    ...
    // Se 404 (dev sem backend), cai no fallback abaixo
    if (res.status !== 404 && res.status !== 405) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.error?.message || `Erro ${res.status}`);
    }
  } catch (e) {
    if (!e.message.includes('Erro')) {
      // Erro de rede — fallback direto
    } else {
      throw e;
    }
  }

  // Fallback: chama Anthropic diretamente (sem API key exposta — funciona apenas se o token for injetado via env no cliente, ou retorna resposta simulada)
  // Em ambiente de desenvolvimento sem backend, retorna resposta offline útil
  return _callIAFallback({ system, messages, max_tokens });
}

async function _callIAFallback({ system, messages, max_tokens }) {
  // Tenta chamar diretamente a API Anthropic (ambiente onde CORS é permitido ou via proxy)
  try {
  ...
  // Resposta offline: analisa a mensagem e retorna orientação genérica
  const userMsg = messages[messages.length - 1]?.content || '';
  let reply = '';

  if (/freelancer|contrat/i.test(userMsg)) {
    reply = '**Contrato de Freelancer**\n\nPara contratar um freelancer, use o modelo "Contrato de Freelancer" na categoria Contratos. Preencha os dados de ambas as partes, descreva detalhadamente o serviço, defina valor e prazo de entrega, e escolha as cláusulas adicionais que se aplicam ao seu caso (ex: Propriedade Intelectual, Sigilo).';
  } else if (/rescis/i.test(userMsg)) {
    reply = '**Cláusula Rescisória**\n\nA cláusula rescisória define as condições para encerramento antecipado do contrato. No campo "Prazo para rescisão", indique o aviso prévio (ex: 30 dias). A multa rescisória é um percentual do valor total do contrato cobrado pela parte que descumprir sem justa causa.';
  } else if (/lgpd/i.test(userMsg)) {
    reply = '**LGPD — Lei Geral de Proteção de Dados**\n\nA LGPD (Lei nº 13.709/2018) regula o tratamento de dados pessoais. Inclua a cláusula LGPD em contratos quando houver coleta ou tratamento de dados do contratado. Você pode ativá-la nas "Cláusulas Adicionais" (passo 5 do wizard).';
  } else if (/foro/i.test(userMsg)) {
    reply = '**Foro Competente**\n\nO foro é a cidade/comarca onde eventuais disputas judiciais serão resolvidas. Normalmente se usa o foro do domicílio do prestador de serviços ou da sede da empresa contratante. Ex: "Comarca de São Paulo, SP".';
  } else if (/pj|clt/i.test(userMsg)) {
    reply = '**PJ vs CLT**\n\nCLT: vínculo empregatício com carteira assinada, férias, 13º salário e FGTS. PJ: o profissional emite nota fiscal como pessoa jurídica, sem vínculo empregatício. Para contratação PJ, use o modelo "Trabalho PJ / Autônomo" na categoria Contratos.';
  } else {
    reply = 'Olá! Sou o assistente do DocFácil. Posso te ajudar a:\n\n• Entender como preencher cada campo dos modelos\n• Explicar cláusulas e termos jurídicos\n• Indicar qual modelo usar para cada situação\n• Dar sugestões de redação\n\n⚠️ *No momento estou operando em modo offline. Para respostas com IA completa, acesse via Vercel ou Netlify com o backend configurado.*\n\nFaça sua pergunta e tentarei ajudar!';
  }
