...
function resetWizard() {
  ...
  // Restaura visibilidade de tudo
  const show = id => { const el = document.getElementById(id); if (el) el.style.display = ''; };
  const hide = id => { const el = document.getElementById(id); if (el) el.style.display = 'none'; };

  show('parte-b-section');
  show('testemunhas-section');
  show('prazo-fields');
  show('step-2-default-content');
  show('step-3-default-content');
  hide('step-2-custom-content');
  hide('step-3-custom-content');
...
function applyWizardRules(typeId) {
  ...
  // ── Roles das partes ──
  const roleALbl = document.getElementById('role-a-label');
  const roleBLbl = document.getElementById('role-b-label');
  if (roleALbl) roleALbl.textContent = getRoleA(typeId);
  if (roleBLbl) roleBLbl.textContent = getRoleB(typeId);

  // ── Parte B — mostra ou oculta ──
  ...
  // ── Steps visíveis na barra de progresso ──
  ...
  // ── Formulário customizado via DocFormularios (se disponível) ──
  // Só atualiza os containers customizados se o módulo estiver carregado
  if (window.DocFormularios?.temFormEspecifico(typeId)) {
    ...
    // Injeta conteúdo customizado no step-2
    window.DocFormularios.renderStep(typeId, 2, 'step-2-custom-content');
    if (s2custom)  s2custom.style.display  = '';
    if (s2default) s2default.style.display = 'none';

    // Injeta no step-3 se o tipo tiver configuração de step3
    if (customCfg?.step3) {
      window.DocFormularios.renderStep(typeId, 3, 'step-3-custom-content');
    ...
    // Sem formulário customizado — garante que o padrão está visível
    const s2custom  = document.getElementById('step-2-custom-content');
    const s2default = document.getElementById('step-2-default-content');
    const s3custom  = document.getElementById('step-3-custom-content');
    const s3default = document.getElementById('step-3-default-content');
